...
class WallCardsController extends SearchPageController
{
    ...
    public function index(
        ...
        $cards = $cardsRepo->search($filters, $pagination->getIndex(), 10000, $searchParams["orderby"]);

        # Ganesh: Get details of all the cards for the corresponding regions
        /*
        $allCards = $cardsRepo->search($filters, $pagination->getIndex(), null, $searchParams["orderby"]);
        ...
        foreach($allCards as $card)
        {
            $mapCards[] = [
                'id' => $card->getId(),
                'name' => $card->getName(),
                'lat' => $card->getAddress()->getLatitude(),
                'long' => $card->getAddress()->getLongitude()
            ];
        }
        */
        #End of mapping
