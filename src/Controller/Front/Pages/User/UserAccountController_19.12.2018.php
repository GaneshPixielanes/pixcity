...
class UserAccountController extends Controller
{
     ...
     */
    public function settings(Request $request, UserPasswordEncoderInterface $passwordEncoder, NotificationsRepository $notificationsRepository)
    {
        $user = $this->getUser();
        ...
        return $this->render('front/account/user/settings.html.twig', array(
            ...
            'notifications' => $notificationsRepository->search('voyager',$this->getUser()->getId())
        ));
     ...
     */
    public function pixies(Request $request, UserRepository $userRepo, StaticPagesRepository $staticPage, NotificationsRepository $notificationsRepository)
    {
        $user = $this->getUser();
     ...
     */
    public function cards(Request $request, CardRepository $cardRepo, CardCategoryRepository $categoriesRepo, RegionRepository $regionsRepo, StaticPagesRepository $staticPages, NotificationsRepository $notificationsRepository)
    {
        $user = $this->getUser();
        $uri = explode('/',$request->getRequestUri())[3];
        ...
        //-----------------------------------------------
        // Create the pager
        $attr = $staticPages->findBy(['url'=>$uri])[0];
//        dd($attr);
        $page = new Page();
     ...
     */
    public function collections(Request $request, CardRepository $cardRepo, CardCollectionRepository $collectionRepo, CardCategoryRepository $categoriesRepo, RegionRepository $regionsRepo, StaticPagesRepository $staticPageRepo, NotificationsRepository $notificationsRepository)
    {
        $user = $this->getUser();
        $uri = explode('/',$request->getRequestUri())[3];
