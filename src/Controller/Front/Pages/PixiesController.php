...
/**
 * @Route("/old-citymaker", name="old_front_pixies_")
 */
...
class PixiesController extends SearchPageController
{
    ...
    public function index(
        ...
        if($request->attributes->get("slug") && $request->attributes->get("slug") !== "france") {
            ...
            // $pixies = $usersRepo->searchPixies($filters);
            $pixies = $userRepo->searchPixiesByRegion($filters);
        }
    ...
    /**
     * @Route("/tous-nos-pixies-locaux-{slug}", defaults={"slug"=null})
     */
    public function pseudoLink(
        Request $request)
    {
        $url = $this->generateUrl('front_pixies_index').'-'.$request->attributes->get("slug");
