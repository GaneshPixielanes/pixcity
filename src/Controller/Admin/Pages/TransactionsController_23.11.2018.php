...
//            dd($project);
        ...
        $this->_createNewTransaction($transactions->getProjects(), $cost, $transactions->getType(), $transactions->getStatus(), $transactions->getUser());

        $this->addFlash('success', 'Invocie has been regenerated');
        return $this->redirectToRoute("admin_transactions_list");
    }

    private function _createNewTransaction($unpayedProjects, $totalPayementPending, $type, $status, $user){


        $em  = $this->getDoctrine()->getManager();
        // Create the new transaction
        $transaction = new Transaction();
        $transaction->setName("C".$user->getId()."-".date('Y').'-'.date('m')."-REGENERATED-".rand(0000,9999));
