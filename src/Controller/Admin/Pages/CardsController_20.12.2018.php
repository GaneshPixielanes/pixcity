...
class CardsController extends Controller
{
     ...
     * @Method({"GET", "POST"})
     */
    public function edit(Request $request, Card $card, Mailer $mailer)
    {
...
//            dd($card->getThumb()->getUrl());
            ...
            if(count($form->getData()->getPixie()->getCards()) == 1 && $form->get('status')->getData() == 'validated')
            {
                ...
                $mailer->send($form->getData()->getPixie()->getEmail(), 'Mail validation 1ere CARD', 'emails/pixie-card-validated-first-time.html.twig', [
                    ...
                    'cardName' => $form->get('name')->getData(),
                    'regionName' => $form->get('region')->getData()->getName(),
                    'slug' => $card->getSlug(),
                    'card' => $card
                ]);
            }
            elseif($form->get('status')->getData() == 'validated')
            {
                //Send mail if the card is validated & this is not user's first card
                $mailer->send($form->getData()->getPixie()->getEmail(), 'Bravo ta card a été mise en ligne!', 'emails/pixie-card-validated-success.html.twig', [
                    'firstName' => $form->getData()->getPixie()->getFirstname(),
                    'cardName' => $form->get('name')->getData(),
                    'regionName' => $form->get('region')->getData()->getName(),
                    'slug' => $card->getSlug(),
