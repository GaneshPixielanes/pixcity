...
class TransactionsController extends Controller
{
    ...
    public function paidByBankTransfer(Request $request, Transaction $transaction, EntityManagerInterface $em, Client $revolut, OptionRepository $optionRepo, Mailer $mailer)
    {
        ...
        if(TransactionStatus::PIXIE_ASKED_BANKTRANSFER_PAYMENT === $transaction->getStatus()) {
            ...
            if($transaction instanceof Transaction) {
                ...
                $invoice_id = $transaction->getName();
                ...
                if ($revolutCounterpartyId && $transaction->getName()) {
                    ...
                    $mailer->send($user->getEmail(), 'Ton paiement a été effectué', 'emails/pixie-card-payment-done.html.twig', [
    ...
    public function invoice(Request $request, TransactionRepository $transactionRepo)
    {
        ...
        if(1 === count($transactions)){
            ...
            {
                $invoicePath = "../invoices/archive/".$invoiceFile;
            }
    ...
    /**
     * @Route("/delete/{slug}",name="invoice_delete")
     */
    public function delete_transaction($slug, TransactionRepository $transactionRepository)
    {
        $filters = ['id' => $slug];

        $transactions = $transactionRepository->search($filters);
        ...
        if(!empty($transactions))
        {
            ...
            {
//            dd($transaction->getHistory());
                if($transaction->getStatus() == 'pixie_asked_banktransfer')
                {
                    foreach($transaction->getHistory() as $history)
                    {
                        $transaction->removeHistory($history);
                    }
//                $transactionHistory = $this->getDoctrine()->getRepository(TransactionHistory::class)->find($transaction->getHistory()->getId());
...
//                $transaction->setStatus(0);
            ...
            $this->addFlash('success', 'Invoice and Transaction details has been deleted!');
            return $this->redirectToRoute("admin_transactions_list");
        }
        else
        {
    ...
    /**
     * @Route("/regenerate/{id}",name="regenerate")
     */
    public function regenerate_transaction($id, TransactionRepository $transactionRepository)
    {
        $fileSystem = new Filesystem();
//        $em  = $this->getDoctrine()->getManager();

        $transactions = $transactionRepository->search(['id' => $id])[0];
        $newName = $transactions->getName().'-OLD-'.rand(0000,9999);
        $oldName = $transactions->getName();
        if(!$fileSystem->exists('../invoices/archive/'.$transactions->getName().'.pdf'))
        {
            $fileSystem->mkdir('../invoices/archive/');
        }
        try{
            $fileSystem->rename('../invoices/'.$transactions->getName().'.pdf', '../invoices/archive/'.$newName.'.pdf');
        }
        catch(Exception $e)
        {
            $this->_createNewTransaction($transactions);
        }
//        $transactions->setName($newName);
...
//        dd($transactions->getName());
        $cost = 0;
        #   Get the total cost
//         foreach($transactions->getProjects() as $project)
//         {
//             $cost = $cost + $project->getPrice();
// //            dd($project);
//         }
        ...
        $this->addFlash('success', 'Invocie has been regenerated');
        return $this->redirectToRoute("admin_transactions_list");
    }

    private function _createNewTransaction($transaction){
       /*        $em  = $this->getDoctrine()->getManager();
        // Create the new transaction
        $transaction = new Transaction();
        if(is_null($name))
        {
            $transaction->setName("C".$user->getId()."-".date('Y').'-'.date('m'));
        }
        else
        {
...
//            dd($e);
        ...
        $this->transactionLog($transaction, "Pixie (".$this->getUser()->getId().") created Regenerated as Transaction");
        // Send mail to admins
//        $mailer->send($this->getParameter("email_admin"), 'Nouvelle demande de paiement', 'emails/admin-card-payment-request.html.twig', [
//            'pixie' => $user()->getFirstname()." ".$user->getLastname()
//        ]);
